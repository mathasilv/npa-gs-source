...
class UDPForwarder(gr.basic_block):
    ...
    def __init__(self, host='localhost', port=5005, filter_bytes=None):
        """
        Args:
        ...
        print(f"Info: Encaminhando para {host}:{port} | {filter_msg}")
        ...
        try:
            ...
            if pmt.is_u8vector(payload):
                ...
                if clean_data:
                    ...
                    print(f"[RX] Total: {len(data_bytes)}b | Payload: {len(clean_data)}b | {status_msg}")
                    # Limita o print do hex para não poluir console se for muito grande
                    display_hex = hex_str if len(hex_str) < 100 else f"{hex_str[:50]}...{hex_str[-10:]}"
                    print(f"HEX: {display_hex}")
...
class RadioBackend(gr.top_block):
    ...
    def __init__(self, freq, sf, bw, gain, bias_tee, host, port, filter_hex=None):
        ...
        if filter_hex:
            try:
                ...
                print(f"Aviso: Filtro hexadecimal inválido '{filter_hex}'. Iniciando sem filtro.")
        ...
        print("--- Configuração SDR ---")
        print(f"Freq: {freq/1e6} MHz")
        print(f"SF: {sf} | BW: {int(bw)} Hz")
        print(f"Gain: {gain} dB | Bias: {bias_tee}")
        print("------------------------")
...
def main():
    ...
    try:
        ...
        print(f"Capturando em {args.freq} MHz...")
        tb.start()
        tb.wait()
